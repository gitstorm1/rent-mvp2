...
export function LedgerClient({ bills }: { bills: any[] }) {
  ...
  return (
    <div>
              ...
              {bills.length === 0 ? (
                ...
                bills.map(bill => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-4 font-medium text-slate-900">{bill.tenants.name}</td>
                      <td className="px-6 py-4 text-slate-600">{bill.tenants.properties.name}</td>
                      <td className="px-6 py-4 text-slate-600">{bill.bill_type}</td>
                      <td className="px-6 py-4 text-slate-600">{bill.billing_month}</td>
                      <td className="px-6 py-4 text-slate-900">${Number(bill.amount_due).toFixed(2)}</td>
                      <td className="px-6 py-4 font-semibold text-slate-900">${balance.toFixed(2)}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        {bill.status !== 'paid' && (
                          ...
                          >
                            ...
                            {sendingReminder === bill.id ? 'Sending...' : 'Remind'}
                          </button>
                        )}
                      </td>
                    </tr>
      ...
      {selectedBill && (
        <PaymentModal bill={selectedBill} onClose={() => setSelectedBill(null)} />
      )}
    </div>
  )
}
